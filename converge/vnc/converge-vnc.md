...
!!! tip
    1. 图形脚本里`module load compiler/intel/2020.1.217 mpi/intelmpi/2020.1.217` 走图形计算并不会识别相关环境参数要在`/Path-to/Convergent_Science/CONVERGE_CFD/4.1.2/environment/x64/scripts/CONVERGE/CONVERGE-INTEL/4.1.2.sh`[配置](#并行配置文件)
    2. 图形闪退的话启动参数加上`-o` 
...
!!! tip
    `*cvg`是保存的图形配置文件
    `*.in *.dat`是输入文件
    `*.echo、*.out、outputs_original`等是输出文件 
